...
- [ ] Validator failed with 51 media errors right after the Brain in CI: the first validate pass ran before `prepare_assets.py`, so it probed raw Pexels downloads (25 fps, audio, 1366 px) against the prepared contract. The pre-prepare pass now uses `--skip-media`; the post-prepare pass checks media.
- [ ] Sourcing took 20 min for 34 beats at a fixed 12 s Pexels wait (44 calls). The Pexels pacer is now adaptive: 2 s between calls, jumps to 12 s after a real 429, relaxes 1 s per success.
- [ ] `prepare_needs` error "timeline references media for beats with no chosen asset": the Brain borrows pool assets / alternates for beats without a pick, so media is not always the beat's `chosen`. Preparation is now driven by `prepare-needs.json` (every media the timeline references, per beat) and writes `prepared/manifest.json` keyed `beatId` or `beatId:assetId`; the Brain's `srcFor()` reads that manifest. A `preparedPath` is never reused across beats (different trims). The workflow runs Brain -> validate(--skip-media) -> prepare-needs -> prepare_assets -> Brain -> validate.
- [ ] Sourcing wall clock is dominated by downloading and CLIP-scoring every candidate video of hard beats (100-200 s per beat that ends with no pick), not by Pexels pacing. Pexels candidates are now pre-scored on their thumbnail (`thumb_url`) and skipped when the thumbnail scores more than 0.06 under the threshold.
- [ ] Remotion chunks failed with 404 on `public/music/*.mp3` / `public/sfx/*.mp3`: SFX and music live in the package `assets/`, not the job dir that is the public dir, and the renderer downloads every `<Audio>` even under `--muted`. Main now renders audio only when the `renderAudio` prop is true (Studio); CI segments are video-only and `audio_mix.ts` builds the mix (spec 14).
- [ ] Audio mix failed with `adelay: Delay must be non negative number`: the mixer swapped in the Brain's `audio-mix.json` items (ms-shaped, `delayMs`) as if they were timeline items, so every `from` was undefined. `audio_mix.ts` now reads sfx/music/narration from `timeline.json` only (spec 14) and takes just ducking parameters from `audio-mix.json`; a negative or missing sfx frame is a hard error.
- [ ] `render-only` now restores the previous run's `job-<id>` artifact (assets.json, prepared/, transcript, plans) so a finish-stage fix does not repeat sourcing.
- [ ] Empty typographic card: when the Director itself prefers `typographic-card` for a beat that has an asset, no key phrase was set, so the card rendered blank. The Brain now derives the key phrase for every typographic card and strips leading/trailing punctuation from it.
- [ ] Three 1366 px-wide Pexels clips were flagged `sd` and forced into `pip-over-blur` with a vintage grade (rule 2 is meant for Internet Archive film). `sd` is now set only for archive.org video under 720 lines.
- [ ] `pip-over-blur` rendered landscape media as a small window on a near-black plate. The window now takes about 84 % of the width for landscape media (portrait media keeps a tall centred window via `MediaRef.portrait`), and the blurred plate is brighter.
