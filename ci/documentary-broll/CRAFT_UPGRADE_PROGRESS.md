...
- [~] Vendored ci/ copies synced and diffed clean — ONGOING, re-checked after each category. CORRECTION found during the Color category: render.yml's 5 directly-invoked scripts are not the FULL CI footprint — replay-media-choices.mjs spawns download-clip.mjs as a subprocess (confirmed via grep), a real TRANSITIVE dependency my earlier "5 scripts + transitions.json" list missed. Did a proper transitive-dependency sweep of all 5 direct scripts this checkpoint (grepping for join()/spawnSync()/import of other .mjs files) — download-clip.mjs was the only miss, now synced; everything else's dependencies were already accounted for (lib/run-log.mjs). gen-cinematic-assets.mjs and transitions.json (synced in the Editing Rhythm checkpoint) plus download-clip.mjs (synced this checkpoint) are the only 3 files that have ever needed a sync. Still not marked [x]: re-diff once more after NLE-parity ships, since that category may touch transitions.json again.
