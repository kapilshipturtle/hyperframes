...
//
//   node transitions.mjs inject --storyboard ./STORYBOARD.md --hyperframes . [--registry <path>]
//   node transitions.mjs verify --storyboard ./STORYBOARD.md --index ./index.html
//
// --registry overrides the vendored ./lib/transitions.json this file ships
// with — for a caller (a different skill) that needs a type outside this
// registry's 5 (e.g. documentary-broll's whip-pan) without mutating this
// shared file, which 4 workflows reuse as-is. Omit it and behavior is
...
function runInject(argv) {
  ...
  const registryPathOverride = flag(argv, "registry", null);
  const manifest = parseStoryboard(readFileSync(storyboardPath, "utf8"));
  const { width: CW, height: CH } = parseFormat(manifest.globals.format);
  const reg = registryPathOverride
    ? loadTransitionRegistry(resolve(registryPathOverride))
    : loadTransitionRegistry();
  const byName = registryPathOverride
    ? transitionsByName(resolve(registryPathOverride))
    : transitionsByName();
